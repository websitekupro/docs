...
With this configuration, all the Deployments, StatefulSets, Jobs, and development containers launched in your namespace will automatically use your registry credentials when pulling private images.
...
We recommend you add the Okteto CLI to your PATH and run the 'okteto context' command to connect your CLI to your Okteto instance before executing the `okteto kubeconfig` command. You can also optionally download your Kubeconfig from the Okteto UI. Please refer to [our documentation](https://www.okteto.com/docs/getting-started/#installing-okteto-cli) for more information on this topic.

Once you have the CLI installed you have to connect to your instance using the command `okteto context use https://okteto.example.com` as is described [here](https://www.okteto.com/docs/getting-started/#configuring-okteto-cli-with-okteto). If you're not logged into Okteto yet, it will also run the login sequence.

Once your Okteto context is configured to access Okteto, you should be able to connect to your Kubernetes cluster using the kubeconfig file generated by Okteto or generate a new one running [`okteto kubeconfig`](https://www.okteto.com/docs/reference/cli/#kubeconfig).
